...
const NewsCard = ({ date, heading, text, linkText, link, src, bigCard }) => {
    return (
        <div className='flex flex-wrap  sm:gap-y-[25px] gap-y-[20px] items-center lg:gap-x-[2.08333333333vw]'>
            <div className={`relative ${bigCard ? "lg:w-[27.0833333333vw] lg:h-[24.375vw]" : "lg:w-[19.7916666667vw] lg:h-[17.8125vw]"} sm:w-[70%] w-full h-[250px] sm:h-[350px]`}>
                <Image fill src={`${src}`} alt='news' className='border20 object-cover' />
            </div>
            <div className='lg:w-[19.7916666667vw] w-full'>
                <span className='text18'>{date}</span>
                <h4 className='text29 f700 mt12 line-clamp-2 lg:h-[3.75vw]'>{heading}</h4>
                <p className='text22 text-[#132D2B] mt16 line-clamp-[5] lg:h-[6.77083333333vw] text-opacity-[0.7]'>{text}</p>
                <Link href={link} className='newsLink text-[#5EBD8E] mt32 flex items-center lg:items-end lg:gap-x-[0.52083333333vw] sm:gap-x-[10px] gap-x-[8px]'>
